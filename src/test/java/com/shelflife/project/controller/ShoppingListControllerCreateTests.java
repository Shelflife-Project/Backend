...
@ActiveProfiles("test")
@Transactional
public class ShoppingListControllerCreateTests {
    ...
    void successfulAddAsOwner() throws Exception {
        ...
        mockMvc.perform(post("/api/storages/" + testUserStorage.getId() + "/shoppinglist")
                .cookie(jwtCookie)
                .contentType(MediaType.APPLICATION_JSON)
                .content(validJson(testProduct.getId(), 10)))
    ...
    @Test
    void invalidProduct() throws Exception {
        String jwt = jwtService.generateToken(testMember.getEmail());
        Cookie jwtCookie = new Cookie("jwt", jwt);
    ...
    void invalidAmount() throws Exception {
        ...
        mockMvc.perform(post("/api/storages/" + testUserStorage.getId() + "/shoppinglist")
                ...
                .content(validJson(testProduct.getId(), -10)))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.amountToBuy").exists());
    }
    ...
    private String validJson(final long productId, final long amountToBuy) {
        return "{\"productId\":" + productId + ", \"amountToBuy\":" + amountToBuy + "}";
    }
}
