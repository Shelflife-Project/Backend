...
@ActiveProfiles("test")
@Transactional
public class ShoppingListControllerEditTests {
    ...
    void successfulEditAsOwner() throws Exception {
        ...
        mockMvc.perform(put("/api/storages/" + testUserStorage.getId() + "/shoppinglist/" + testItem.getId())
    ...
    void itemNotFound() throws Exception {
        ...
        mockMvc.perform(put("/api/storages/" + testUserStorage.getId() + "/shoppinglist/" + testItem.getId() + 10)
                .contentType(MediaType.APPLICATION_JSON)
                .content(validJson(5))
                .cookie(jwtCookie))
    ...
    private String validJson(final long amountToBuy) {
        return "{\"amountToBuy\":" + amountToBuy + "}";
    }
}
